...
For every tool call, the server reads credentials from the process environment, then from the explicit `worktree_path` when present, `DEBOAI_REPOSITORY_ROOT` when set, and finally the server working directory. Each directory contributes `.env` followed by `debo.env`; earlier values win and env files never modify the server process. Copy [configs/example.env](configs/example.env) as a starting point:
...
The server speaks JSON-RPC over stdio. Every tool accepts an optional `worktree_path`, allowing one long-lived server to inspect multiple worktrees. It defaults to MCP protocol revision `2026-07-28`, negotiating down to `2025-06-18`, `2025-03-26`, or `2024-11-05` for clients that request one of those. Register it with your assistant, for example in `.mcp.json`:
